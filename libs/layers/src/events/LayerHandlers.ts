import { NodeId, Node, DerivedCoreEventHandlers } from '@libs/core';
          ...
          if (indicatorInfo) {
            ...
            if (editorStore.query.node(parent.id).isCanvas()) {
              if (parent.data.parent) {
                const grandparent = editorStore.query.node(parent.data.parent).get();
                if (editorStore.query.node(grandparent.id).isCanvas()) {
                  LayerHandlers.events.currentCanvasHovered = parent;
                  if (
                    (e.clientY > parentHeadingInfo.bottom - 10 && !this.getLayer(parent.id).expanded) ||
                    ...
                    indicatorInfo.placement.index = grandparent.data.nodes
                      ...
                      : 0;
                    if (e.clientY > parentHeadingInfo.bottom - 10 && !this.getLayer(parent.id).expanded) {
