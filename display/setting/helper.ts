  ...
  const generatedSections = sections.map((section) => {
    if (typeof section === "string") {
      ...
      if (DEFAULT_SECTIONS[sectionName]) {
        const { items: defaultItems, ...otherDefaultSectionProps } =
          DEFAULT_SECTIONS[sectionName];
        ...
        if (!items || items.length === 0) {
          ...
          finalItems = items.map((item) => {
            if (typeof item === "string") {
              const foundDefaultItem = generatedDefaultItems.find(
                (defaultItem) => defaultItem.propKey === item
              );
              ...
              const foundDefaultItem = generatedDefaultItems.find(
                (defaultItem) => {
                  if (item.hasOwnProperty("index")) {
                    ...
                    return (
                      defaultItem.propKey === item.propKey &&
                      defaultItem.index === item.index
                    );
